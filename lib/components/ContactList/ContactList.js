        ...
        return (React.createElement("div", { className: "page-container", style: hasTopBar ? { marginTop: '47px' } : {} }, (function () {
            if (contacts && contacts.length > 0) {
                ...
                for (var i = 0; i < contacts.length; i++) {
                    roomItems.push(React.createElement(_1.CheckListItem, { key: 'contact-list-item-' + i, name: contacts[i].name, pictureUrl: contacts[i].pictureUrl, width: 40, height: 40, onClick: _this.onClick.bind(_this, contacts[i]), isChecked: (selectedContacts[contacts[i].userId]) ? true : false }));
                }
                return roomItems;
            }
            else {
